...
def load_data():
    ...
    data = np.reshape( data, [-1, 20, 64, 64, dim] )
...
X = tf.placeholder(tf.float32, [batch_size, frames, 64, 64, dim])
Y = tf.placeholder(tf.float32, [batch_size, frames, 64, 64, dim])
Y_future = tf.placeholder(tf.float32, [batch_size, frames, 64, 64, dim])


c0 = tf.reshape(X,[-1, 64, 64, dim])
...
img_pre = np.reshape( img_pre, [batch_size, frames, 64, 64, dim] )
img = np.reshape( img, [batch_size, frames, 64, 64, dim] )
img_future = np.reshape( img_future, [batch_size, frames, 64, 64, dim] )
